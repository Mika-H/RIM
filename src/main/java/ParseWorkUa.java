...
//    }
    public LinkedHashSet<String> vacancyFilter(LinkedHashSet<String> links){
        ...
        Pattern p = Pattern.compile("(/jobs/[0-9]+)$");
        Matcher m;
        while(link.hasNext()){

            inspectLink = tempLink = link.next();
            if(tempLink.endsWith("/")&&!tempLink.equals("")){
                tempLink=tempLink.substring(0, tempLink.length()-1);  //удаление последнего символа если он "/"
            }
            startDesignSeach = tempLink.lastIndexOf("/");
            if(startDesignSeach>9){    // так как любая ссылка должна начинаться с robota.ua, то мы эту часть ссылки не рассматриваем
                tempLink = tempLink.substring(startDesignSeach-5);
                m = p.matcher(tempLink);
                if(m.matches()){
